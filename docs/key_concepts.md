...
| ----- | ------- | ------------- |
| `atom_pairs` | (N, 5) | Integer matrix where N is the number of edges. Each row contains 5 integers: `[R1, R2, R3, i_atom, j_atom]`, representing a coupling between the $n_1$-th atom in the central unit cell and the $n_2$-th atom in the periodic image cell specified by the lattice vector indices $(i, j, k)$.  |
